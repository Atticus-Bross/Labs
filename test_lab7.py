...
def test_add_alignment() -> None:
    ...
    assert add_alignment(['word', 'test', 'cube', 'some'], ['left', 'right', 'left', 'right']) == [':word', 'test:',
                                                                                                   ':cube', 'some:']
...
def test_table_row() -> None:
    ...
    assert table_row([':abc', 'efg:', ':hij', 'lmn:'], [5, 8, 6, 9],
                     ['left', 'right', 'left', 'right']) == '|:abc |    efg:|:hij  |     lmn:|\n'
...
def test_table_data() -> None:
    ...
    assert table_data(['Test', 'tester', 1, 'a', None, 'b'], 2) == (
    ['Test', 'tester', '1', 'a', '', 'b'], ['right', 'left'], 2)
...
|    |    ...\n"""
    with io.StringIO() as test_output:
        view_table({10: 'Biiiiiig', True: 'test', 'a': 'square', 2.2: 'ccc'},
                   [{'a': 34, True: 'asd', 2.2: False, 10: 2.34534}], 20, test_output)
...
def test_merge_sorted_lists() -> None:
    ...
    assert merge_sorted_lists([1.1, 2.2, 3.3, 4.4], [0.2, 1.3, 3.7], [1.7, ]) == [0.2, 1.1, 1.3, 1.7, 2.2, 3.3, 3.7,
                                                                                  4.4]
...
with open('test.md', 'w') as mdfile:
    if function_to_test == 'table':
        ...
        mdfile.writelines(
            table(['a', 'ab', 'abc', 'abcd', 'abcde', 'abcdef', 'as', 'asd', 'asdf'], ['right', 'left', 'left'], 3))
        mdfile.writelines(
            table(['a', 'b', 'ab', 'bc', 'abc', 'bcd', 'abcd', 'bcde'], ['left', 'right', 'right', 'left'], 2))
    elif function_to_test == 'table_from_list':
        mdfile.writelines(
            table_from_list(['test', 'tester', 'te'], [[None, None, None], [1, 'a', 1.23456], [1, 'b', 3.5677]]))
        mdfile.writelines(
            table_from_list([True, 'tester', 'te'],
                            [[1.2341892402, 'asdfkjls;dhfl', 334578937], [1.24332, 'a', None], [1.345, None, 3]]))
        mdfile.writelines(
            table_from_list(['t', 't', 't', 'boolsbools'],
                            [[1, 'a', 1.23456, False], [None, None, None, None], [1, 'b', 3.5677, True]]))
        ...
        mdfile.writelines(table_from_dict([0, True, 2.3433, 'abcde'],
                                          [{2.3433: 8, 'abcde': 'asd'}, {0: True, True: 2.345},
                                           {0: False, True: 23.345, 2.3433: 5, 'abcde': 'fgh'}]))
        mdfile.writelines(table_from_dict([0, True, 2.3433, 'abcde'],
                                          [{2.3433: 8, 'abcde': 15651615615}, {0: True, True: 2.345},
                                           {0: False, True: 23.345, 2.3433: 5, 'abcde': 0}]))
        mdfile.writelines(table_from_dict({10: 'Biiiiiig', True: 'test', 'a': 'square', 2.2: 'ccc'},
                                          [{'a': 34, True: 'asd', 2.2: False, 10: 2.34534}]))
        mdfile.writelines(table_from_dict({10: 'Biiiiiig', True: 'test', 'a': 'square', 2.2: 'ccc'},
